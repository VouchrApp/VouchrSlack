...
export class ValidationService {
    ...
    validateRequest(signingSecret: string, signingInfo: SigningInfo): void {
        ...
        const baseString: string = [this.version, signingInfo.timestamp, signingInfo.getBody()].join(':');
        if (signingInfo.timestamp) {
            ...
            if (currentTime > timeStampToMilli) {
                ...
                const calculatedSignature = crypto.createHmac(this.signatureType, signingSecret)
                    .update(baseString)
                    .digest(this.digestEncoding);
                if (crypto.timingSafeEqual(Buffer.from(signingInfo.signature), Buffer.from(`v0=${calculatedSignature}`))) {
